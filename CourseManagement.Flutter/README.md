...
ثبت Flutter من قناة stable. يدعم المشروع الحالي Flutter 3.35.7 مع Dart 3.9.2، لذلك لا تحتاج إلى تنزيل ترقية كاملة لـFlutter حتى تشغله. تم تثبيت إصدارات `file_picker` و`video_player` و`go_router` و`flutter_secure_storage` المتوافقة مع هذه البيئة. يمكن فحص البيئة بالأمر التالي، ثم تشغيل التطبيق من هذا المجلد. توصي وثائق Flutter باستخدام أوامر CLI نفسها للتحقق والبناء والتشغيل [1].
