...
| أصول الكورسات | Instructor/Admin يختار فيديو أو مرفق من الجهاز ويرفعه ويديره، والطالب المسجّل يشغّل الفيديو وينزّل المرفق | `GET/POST/DELETE /api/course/{id}/assets` و`GET .../download` |
...
حدود الملفات يفرضها الخادم: الفيديو `mp4/webm/mov/m4v` حتى 512MB، والمرفقات `pdf/doc/docx/ppt/pptx/xls/xlsx/zip/txt` حتى 50MB. لا تحفظ bytes في SQLite ولا تضع مجلد uploads داخل `build/web` أو `assets`.
